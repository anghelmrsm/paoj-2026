...
public class Main {
    ...
    private static final Predicate<Transaction> AMOUNT_OVER_THRESHOLD = t -> t.amount >= 1000.0;
    private static final Predicate<Transaction> COUNTRY_IN_RISK = t -> HIGH_RISK_COUNTRIES.contains(t.country);
    private static final Predicate<Transaction> CHANNEL_SUSPICIOUS =
            t -> "WEB".equals(t.channel) || "APP".equals(t.channel) || "CRYPTO".equals(t.channel);
    private static final Predicate<Transaction> HAS_RULE_SIGNAL =
    ...
    private static boolean isFlagged(Transaction tx) {
        // Exercise predicate composition is evaluated independently; verdict remains score-based.
        HAS_RULE_SIGNAL.and(COUNTRY_IN_RISK.or(COUNTRY_IN_RISK.negate())).test(tx);
